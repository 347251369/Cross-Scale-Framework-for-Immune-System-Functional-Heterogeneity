...
expr_matrix = as.sparse(subPBMC@assays$RNA@counts)
##提取表型信息到p_ data(phenotype_data) 里面
p_data = subPBMC@meta.data
# p_data$celltype = subPBMC@meta.data ##整合每个细胞的细胞鉴定信息到p_data里面。如果已经)
#提取基因信息如生物类型、gc含量等
f_data = data.frame(gene_short_name = row.names(subPBMC) , row.names = row.names(subPBMC))
...
cds = setOrderingFilter(cds, row.names(subPBMC))
